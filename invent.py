...
"""
The purpose of this module is to use th Nornir inventory class and understand how it works in detail. I will be commenting out each line.
"""
...
class BaseAttributes:
    # Slots is used here to make Nornir more efficient. Every time you create an instance of this class, by default, python create a __dict__ with the keys and values. If you create 1000 hosts, this would create 1000 instances and 1000 dictionaries. Instead, by using slots, this can save memory (40-50% less memory that dicts) as it uses an array-like structure instead of a dictionary.
    ...
    # With a class method, you don't need to create an object/instance to use this method. You can just call BaseAttributes.schema() to return the schema without ever having to create an instance
    @classmethod
    ...
    # By using this Dict Method instead of __dict__ you save memory because python only allocates memory to this method only when it gets executed. In contrast, if we didn't use __slots__ a __dict__ will be created in memory persitenlty everytime a new instace is created, wether you use that dicitonary or not.
    def dict(self) -> Dict[str, Any]:
        return {
            # object.__getattribute__(self, attribute) -  Just gets the value directly from the instance slot instead. This is useful because an attribute like "port" using the nornir business logica can be inherited from the host, or the groups or the defaults. By using this, you don't have to go all the layers of the business logic but go directly to the bottom where the value is stored.
...
class ConnectionOptions(BaseAttributes):
    ...
    """
    # This class is inhereting from BaseAttributes. The __init__ method inherits all attributes from the parents but also defines the same local attributes itself. YOu don't need to redefine them but it is useful for a few reasons:
        1 - IDE Autocomplete and Type Hinting: Users can get autocomplete when writing ConnectionOptions()
        2 - Controlling Argument Order: You can define how arguments would be provided
        3 - Explicit vs. Implicit (Zen of code) instead of having to go to the parent to look for its attributes
    ...
    """ Data Flow
    When you instantiate the following: ConnectionOptions(hostname="DMAC")
        1 - You instantiate the object conn = ConnectionOptions(hostname="DMAC")
        2 - The string DMAC passes to the child ConnectionOptions, to the hostname argument. username: Optional[str] = None,
        3 - Inside super().__init__() --> hostname=hostname,
            3.1 - Left hand side hostname is pointing to the parent's attribute where the value will be stored
            3.2 - Right hand side hostname is the local value containing DMAC. We could change this to another name. It just needs to match in the ConnectionOptions.__init__ arguments.
        4 - The parent Base Attribute will then receive "DMAC" from the child class and will store it in is hostname in the __slot__
    """
    ...
    def schema(cls) -> Dict[str, Any]:
        return {
            "extras": {"$key": "$value"},
            **super().schema(),
            # ** is unpacking the dictionary from the parent BaseAttributes.schema() -> i.E hostname, password, platform,etc,
        }

    """
    This schema method is used for validation and documentation
    """

    # Since this is not a class method, it needs to be called from the instance, not the class
    # Example: cn = ConnectionOptions(extras={"name":"scrapli", "tansp":"ssh"})
    # cn.dict() --> This will return the attreibutes from the parent class (hostname, port, username, pass) and also the extras dict.
    def dict(self) -> Dict[str, Any]:
        return {
            "extras": self.extras,
            **super().dict(),
    ...
    def filter(
        ...
        **kwargs: Any,
        ...
        if filter_func:
            ...
            filtered = Hosts({n: h for n, h in self.hosts.items() if all(h.get(k) == v for k, v in kwargs.items())})
